                            ...
                            <div class="p-4 bg-slate-50 dark:bg-slate-700 rounded-xl cursor-pointer hover:bg-blue-50 dark:hover:bg-slate-600 transition-all border-2 border-transparent hover:border-blue-200 dark:hover:border-blue-800 student-result ${s.disabled ? 'opacity-50 cursor-not-allowed' : ''}" data-id="${s.id}" data-name="${s.name}">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <div class="text-sm text-slate-500">${s.class} • Age: ${s.age || 'N/A'}${s.disabled ? ' <span class="text-red-500 font-bold">(Already linked)</span>' : ''}</div>
                                    </div>
                            ...
                            <tbody class="divide-y">
                                ${state.students.map(s => `
                                    <tr class="text-sm">
                                        ...
                                        <td class="px-4 py-2 text-slate-500">${s.class}</td>
                            ...
                            <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                                ${myStudents.map(s => `
                                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                        <td class="px-6 py-4 font-semibold text-slate-800 dark:text-slate-200">${s.name}</td>
                                        <td class="px-6 py-4">
                                            <span id="attendance-status-${s.id}" class="px-3 py-1 rounded-full text-xs font-bold bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300">
                                                Not Marked
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 text-right">
...
const actions = {
    ...
    async addClass() {
        ...
        try {
            ...
            const { error } = await supabaseClient.from('classes').insert([{ level, grade }]);
    ...
    async approveReportBundle(bundleId) {
        try {
            ...
            if (approvedBundle?.class) {
                const classStudents = state.students.filter(s => s.class === approvedBundle.class);
