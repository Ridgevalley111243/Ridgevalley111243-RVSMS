    ...
    selectStudent(title, students, onSelect) {
        ...
        const resolveClassLabel = (s) => {
            if (s.class_id) {
                ...
                if (classObj) return `${classObj.level} - ${classObj.grade}`;
            }
            return s.class || '—';
        };
        const html = `
...
function setupRealtimeSubscriptions() {
    const channels = [
        ...
        supabaseClient.channel('public:attendance').on('postgres_changes', { event: '*', schema: 'public', table: 'attendance' }, () => actions.refreshData())
    ];
...
const dataManager = {
    ...
    normalizeStudentRecords(students) {
        ...
        // id → class object
        const classById = {};
        // normalised "level - grade" → class object
        const classByLabel = {};
        const normLabel = s =>
            (s || '').trim().replace(/\s{2,}/g, ' ').replace(/\s*-\s*/g, ' - ').toLowerCase();

        state.classes.forEach(c => {
            classById[c.id] = c;
            classByLabel[normLabel(`${c.level} - ${c.grade}`)] = c;
        });
        ...
        return students.map(student => {
            ...
            if (!classObj && s.class && !UUID_RE.test(s.class.trim())) {
                classObj = classByLabel[normLabel(s.class)] || null;
                if (classObj) {
                    s.class_id = classObj.id;
                }
            }
            ...
            if (classObj) {
                const canonicalLabel = `${classObj.level} - ${classObj.grade}`;
                s.class    = canonicalLabel;
                s.class_id = classObj.id;
    ...
    async migrateStudentClassIds() {
        try {
            ...
            state.classes.forEach(c => {
                classById[c.id] = c;
                classByLabel[norm(`${c.level} - ${c.grade}`)] = c.id;
            });
            ...
            for (const student of toFix) {
                ...
                if (student.class && UUID_RE.test(student.class.trim())) {
                    ...
                    if (classObj) {
                        ...
                        newClassLabel = `${classObj.level} - ${classObj.grade}`;
                    } else {
                ...
                if (!newClassId && newClassLabel && !UUID_RE.test((newClassLabel || '').trim())) {
                    newClassId = classByLabel[norm(newClassLabel)] || null;
    ...
    calculateAttendanceStats(studentId) {
        const studentAttendance = state.attendance.filter(a => a.student_id === studentId);
    ...
    getTeacherTotalAttendance() {
        ...
        return classStudents.map(student => {
            ...
            if (student.class_id) {
                ...
                if (classObj) resolvedClass = `${classObj.level} - ${classObj.grade}`;
            }
            return {
...
const ui = {
    ...
    updateSidebar() {
        ...
        const teacherItems = [
            ...
            { id: 'teacher_attendance',       label: 'Attendance',       icon: 'fa-clipboard-check' },
            { id: 'teacher_total_attendance', label: 'Total Attendance', icon: 'fa-calendar-check' },
        ...
        if (nav) {
            nav.innerHTML = items.map(item => {
                ...
                return `
                    <button onclick="ui.route('${item.id}'); ui.toggleSidebar()"
...
const views = {
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:16px;margin-bottom:24px;">
                ${stats.map(s => `
                    <div onclick="${s.action}" style="background:var(--rv-surface);border:1px solid var(--rv-border);border-radius:14px;padding:20px;box-shadow:var(--rv-shadow,0 2px 16px rgba(15,32,68,0.08));cursor:${s.action ? 'pointer' : 'default'};transition:all 0.18s;" onmouseover="if('${s.action}')this.style.transform='translateY(-2px)';this.style.boxShadow='0 8px 24px rgba(15,32,68,0.12)'" onmouseout="this.style.transform='';this.style.boxShadow='var(--rv-shadow)'">
                        ...
                        <div style="font-family:'Outfit',sans-serif;font-size:28px;font-weight:700;color:${s.label.includes('Arrears') ? '#dc2626' : 'var(--rv-text)'};line-height:1;">${s.value}</div>
                                ...
                                <tbody>
                                    ${allArrears.slice(0, 5).map(a => `
                                        <tr style="border-top:1px solid var(--rv-border);">
                                            ...
                                            <td style="padding:10px 12px;text-align:right;font-weight:700;color:#dc2626;">₵${a.amount.toFixed(2)}</td>
                                        </tr>
                                    `).join('')}
                                </tbody>
                            </table>
    ...
    showDetailModal(type) {
        ...
        switch(type) {
            ...
            case 'revenue':
                ...
                content = `
                    <div class="max-h-96 overflow-y-auto">
                        <p class="mb-4 font-bold text-emerald-600 text-xl">Total: ₵${confirmed.reduce((sum, t) => sum + (parseFloat(t.amount) || 0), 0).toLocaleString()}</p>
                        <table class="w-full text-left">
                            <thead class="bg-slate-50 dark:bg-slate-700 border-b">
                                <tr>
                            ...
                            <tbody class="divide-y">
                                ${confirmed.map(t => `
                                    <tr class="text-sm">
                                        ...
                                        <td class="px-4 py-2 font-semibold text-emerald-600">₵${t.amount}</td>
            ...
            case 'arrears':
                ...
                content = `
                    <div class="max-h-96 overflow-y-auto">
                        <p class="mb-4 font-bold text-red-600 text-xl">Total: ₵${arrears.reduce((sum, a) => sum + a.amount, 0).toLocaleString()}</p>
                        <table class="w-full text-left">
                            <thead class="bg-slate-50 dark:bg-slate-700 border-b">
                                <tr>
                            ...
                            <tbody class="divide-y">
                                ${arrears.map(a => `
                                    <tr class="text-sm">
                                        ...
                                        <td class="px-4 py-2 text-right font-bold text-red-600">₵${a.amount.toFixed(2)}</td>
                                    </tr>
                                `).join('')}
                            </tbody>
                        </table>
    ...
    renderStudents() {
        ...
        sortedStudents.forEach(s => {
            // FIX 3: Prefer class label resolved from class_id (UUID) over raw s.class string
            // This prevents raw UUIDs from ever appearing in the UI as group headings.
            let displayLabel = null;
            if (s.class_id) {
                const classObj = state.classes.find(c => c.id === s.class_id);
                if (classObj) displayLabel = `${classObj.level} - ${classObj.grade}`;
            }
    ...
    renderAttendance() {
        const html = `
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-2xl font-bold text-slate-800 dark:text-white">Attendance Records</h2>
                <button onclick="actions.refreshData()" class="px-4 py-2 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-lg text-sm font-bold transition-colors">
                    <i class="fas fa-sync mr-2"></i> Refresh
                </button>
            </div>
            ...
            <div class="glass-panel rounded-2xl p-6 mb-6 bg-white dark:bg-slate-800 shadow-lg">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="date" id="admin-attendance-date" class="input-field w-full rounded-xl px-4 py-3 border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-white focus:ring-2 focus:ring-ridge-500 outline-none" value="${new Date().toISOString().split('T')[0]}">
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-2 text-slate-700 dark:text-slate-300">Select Class</label>
                        <select id="admin-attendance-class" class="input-field w-full rounded-xl px-4 py-3 border border-slate-300 dark:border-slate-600 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-slate-700 dark:to-slate-700 text-slate-800 dark:text-white focus:ring-2 focus:ring-ridge-500 outline-none">
                            <option value="">All Classes</option>
                            ${state.classes.map(c => `<option value="${c.id}">${c.level} - ${c.grade}</option>`).join('')}
                        </select>
                    </div>
                    <div class="flex items-end">
                        <button onclick="actions.loadAdminAttendance()" class="w-full px-6 py-3 bg-gradient-to-r from-ridge-500 to-blue-600 text-white rounded-xl font-bold hover:shadow-lg transition-all">
            ...
            <div id="admin-attendance-results" class="glass-panel rounded-2xl p-6 bg-white dark:bg-slate-800 shadow-lg">
                <div class="text-center py-12 text-slate-500">
                    <i class="fas fa-clipboard-list text-4xl mb-3 text-slate-300"></i>
                    <p>Select date and class to view attendance records</p>
                </div>
            </div>
        `;
                        ...
                        <div class="glass-panel rounded-2xl overflow-hidden bg-white dark:bg-slate-800 shadow-lg border border-slate-200 dark:border-slate-700">
                            ...
                            <div class="p-4">
                                ...
                                <div class="space-y-1 text-sm text-slate-600 dark:text-slate-400 mb-4">
                                    ...
                                    <p><i class="fas fa-money-bill mr-2"></i>Amount: ₵${t.amount}</p>
    ...
    async renderTeacherDashboard() {
        ...
        const avgAttendance = totalAttendance.length > 0 
            ? Math.round(totalAttendance.reduce((sum, s) => sum + s.percentage, 0) / totalAttendance.length) 
            : 0;
        ...
        const html = `
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div class="glass-panel p-6 rounded-2xl border-l-4 border-emerald-500 bg-white dark:bg-slate-800 shadow-lg">
                    ...
                    <h3 class="text-3xl font-black text-slate-800 dark:text-white">${avgAttendance}%</h3>
    ...
    async renderTeacherAttendance() {
        ...
        if (activeWeek) {
            ...
            weekDateOptions = weekDates.map(iso => {
            ...
            if (!weekDates.includes(today)) defaultDate = weekDates[0] || today;
        ...
        const dateSelector = activeWeek ? `
            <div class="mb-6">
                ...
                <select id="attendance-date" class="input-field rounded-xl px-4 py-3 border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-white w-full md:w-auto focus:ring-2 focus:ring-ridge-500 outline-none" onchange="views._onAttendanceDateChange()">
                    ${weekDateOptions}
                </select>
            </div>
        ` : `
            <div class="mb-6">
                <label class="block text-sm font-medium mb-2 text-slate-700 dark:text-slate-300">Select Date</label>
                <input type="date" id="attendance-date" class="input-field rounded-xl px-4 py-3 border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-white w-full md:w-auto focus:ring-2 focus:ring-ridge-500 outline-none" value="${defaultDate}" onchange="views._onAttendanceDateChange()">
            </div>
        `;
        ...
        const html = `
            <div class="glass-panel rounded-2xl p-6 bg-white dark:bg-slate-800 shadow-lg">
                ...
                <div id="attendance-loading-indicator" style="display:none;text-align:center;padding:20px 0;">
                    <i class="fas fa-spinner fa-spin" style="color:#1a56db;font-size:20px;"></i>
                    <p style="color:#64748b;font-size:13px;margin-top:8px;">Loading attendance for selected day...</p>
                </div>

                ${myStudents.length === 0 ? `
                    <p class="text-slate-500 text-center py-8">No students assigned to your class</p>
                ` : `
                    <div id="attendance-table-wrapper" class="overflow-x-auto">
                        <table class="w-full text-left min-w-[600px]">
                            <thead class="bg-slate-50 dark:bg-slate-700 border-b border-slate-200 dark:border-slate-600">
                                <tr>
                                    <th class="px-6 py-4 font-bold text-sm text-slate-700 dark:text-slate-200">Student Name</th>
                                    <th class="px-6 py-4 font-bold text-sm text-slate-700 dark:text-slate-200">Status</th>
                                    <th class="px-6 py-4 font-bold text-sm text-slate-700 dark:text-slate-200 text-right">Action</th>
                                </tr>
                            </thead>
                            <tbody id="attendance-tbody" class="divide-y divide-slate-200 dark:divide-slate-700">
                                ${myStudents.map(s => `
                                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                        <td class="px-6 py-4 font-semibold text-slate-800 dark:text-slate-200">${s.name}</td>
                                        <td class="px-6 py-4">
                                            <span id="attendance-status-${s.id}" data-status="" class="px-3 py-1 rounded-full text-xs font-bold bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300">
                                                Not Marked
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 text-right">
                                            <button onclick="actions.markAttendance('${s.id}', 'present')" class="px-3 py-1 bg-emerald-500 text-white rounded-lg text-xs font-bold mr-2 hover:bg-emerald-600 transition-colors">Present</button>
                                            <button onclick="actions.markAttendance('${s.id}', 'absent')" class="px-3 py-1 bg-red-500 text-white rounded-lg text-xs font-bold hover:bg-red-600 transition-colors">Absent</button>
                                        </td>
                                    </tr>
                                `).join('')}
                            </tbody>
                    ...
                    <button onclick="actions.saveAttendance()" class="mt-6 px-6 py-3 bg-gradient-to-r from-ridge-500 to-blue-600 text-white rounded-xl font-bold hover:shadow-lg transition-all w-full md:w-auto">
        ...
        // After rendering, load attendance for the initially selected date
        if (myStudents.length > 0) {
            await views._loadAttendanceForSelectedDate();
        }
    },

    // ── Attendance date isolation helper ──────────────────────────────────────
    // Called whenever the teacher changes the selected date.
    // Resets all student rows to "Not Marked" then fills in any saved records
    // specifically for that date+class combination.
    async _onAttendanceDateChange() {
        await views._loadAttendanceForSelectedDate();
    },

    async _loadAttendanceForSelectedDate() {
        ...
        // Show loading indicator, hide table briefly
        const loadingEl = document.getElementById('attendance-loading-indicator');
        ...
        // Reset ALL student status badges to "Not Marked" before loading new data
        ...
        try {
            // Fetch attendance records strictly by date + class_id (UUID-based)
            const classString = `${assignedClass.level} - ${assignedClass.grade}`;
            const studentIds = myStudents.map(s => s.id);

            const { data: dayRecords, error } = await supabaseClient
                .from('attendance')
            ...
            // Apply saved attendance for this specific date only
            if (dayRecords && dayRecords.length > 0) {
                dayRecords.forEach(record => {
                    const statusEl = document.getElementById(`attendance-status-${record.student_id}`);
                    if (statusEl) {
            ...
            console.warn('_loadAttendanceForSelectedDate error:', err);
        } finally {
    ...
    async renderTeacherTotalAttendance() {
        ...
        const html = `
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-2xl font-bold text-slate-800 dark:text-white">Total Attendance Summary</h2>
                <span class="text-sm text-slate-500">Cumulative attendance for current term</span>
            </div>

            <div class="glass-panel rounded-2xl overflow-hidden bg-white dark:bg-slate-800 shadow-lg">
                ${totalAttendance.length === 0 ? '<p class="p-8 text-center text-slate-500">No students assigned to your class</p>' : `
                    <div class="overflow-x-auto">
                        <table class="w-full text-left min-w-[600px]">
                            <thead class="bg-slate-50 dark:bg-slate-700 border-b border-slate-200 dark:border-slate-600">
                                <tr>
                                    <th class="px-6 py-4 font-bold text-sm text-slate-700 dark:text-slate-200">Student Name</th>
                                    <th class="px-6 py-4 font-bold text-sm text-slate-700 dark:text-slate-200 text-center">Days Present</th>
                                    <th class="px-6 py-4 font-bold text-sm text-slate-700 dark:text-slate-200 text-center">Days Absent</th>
                                    <th class="px-6 py-4 font-bold text-sm text-slate-700 dark:text-slate-200 text-center">Attendance Rate</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                                ${totalAttendance.map(s => `
                                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                        <td class="px-6 py-4 font-semibold text-slate-800 dark:text-slate-200">${s.name}</td>
                                        <td class="px-6 py-4 text-center text-emerald-600 font-bold">${s.present}</td>
                                        <td class="px-6 py-4 text-center text-red-600 font-bold">${s.absent}</td>
                                        <td class="px-6 py-4 text-center">
                                            <div class="flex items-center justify-center gap-2">
                                                <div class="w-16 bg-slate-200 rounded-full h-2">
                                                    <div class="bg-emerald-500 h-2 rounded-full" style="width: ${s.percentage}%"></div>
                                                </div>
                                                <span class="font-bold ${s.percentage >= 90 ? 'text-emerald-600' : s.percentage >= 75 ? 'text-amber-600' : 'text-red-600'}">${s.percentage}%</span>
                                            </div>
                                        </td>
                                    </tr>
                                `).join('')}
    ...
    try {
        ...
        const classString = assignedClass ? 
            `${assignedClass.level} - ${assignedClass.grade}` : '';
                    ...
                    return `
                        <div class="glass-panel p-6 rounded-2xl card-hover bg-white dark:bg-slate-800 shadow-lg border border-slate-200 dark:border-slate-700">
                            ...
                            <div class="flex justify-between items-center p-3 bg-slate-50 dark:bg-slate-700 rounded-xl">
                                ...
                                <span class="font-bold ${arrears > 0 ? 'text-red-600' : 'text-emerald-600'}">₵${arrears.toFixed(2)}</span>
                            </div>
                        </div>
                    `;
                }).join('')}
                    ...
                    <div class="glass-panel rounded-2xl p-6 bg-white dark:bg-slate-800 shadow-lg border border-slate-200 dark:border-slate-700">
                        ...
                        <div class="grid grid-cols-3 gap-4 text-center">
                            ...
                            <div class="p-3 bg-slate-50 dark:bg-slate-700 rounded-xl">
                                ...
                                <p class="font-bold text-red-600">₵${dataManager.calculateStudentArrears(child.id).toFixed(2)}</p>
                            </div>
                            <div class="p-3 bg-slate-50 dark:bg-slate-700 rounded-xl">
        ...
        const html = `
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div class="glass-panel p-6 rounded-2xl bg-white dark:bg-slate-800 shadow-lg border border-slate-200 dark:border-slate-700">
                    ...
                    <h3 class="text-3xl font-black text-red-600">₵${totalDue.toFixed(2)}</h3>
                </div>
                <div class="glass-panel p-6 rounded-2xl bg-white dark:bg-slate-800 shadow-lg border border-slate-200 dark:border-slate-700">
                    ...
                    : `<div class="space-y-3">
                        ${relevantFees.map(fee => `
                            <div class="flex items-center justify-between p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-200 dark:border-blue-800">
                                ...
                                <span class="text-lg font-black text-blue-600 dark:text-blue-400">₵${fee.amount}</span>
                            </div>
                        `).join('')}
                    </div>`
                }
                                    ...
                                    <div class="p-4 bg-slate-50 dark:bg-slate-700 rounded-xl border border-slate-200 dark:border-slate-600">
                                        <div class="flex items-start justify-between mb-2">
                                            <div>
                                                ...
                                                <p class="text-sm text-slate-500">₵${t.amount} • ${t.method === 'momo' ? 'Mobile Money' : 'Cash'}</p>
...
const actions = {
    ...
    async addClass() {
        ...
        const norm = s => (s || '').trim().replace(/\s{2,}/g, ' ').replace(/\s*-\s*/g, ' - ').toLowerCase();
        const inputKey = norm(`${level} - ${grade}`);
        ...
        try {
            ...
            const match = (deletedClasses || []).find(
                c => norm(`${c.level} - ${c.grade}`) === inputKey
            );
    ...
    async loadAdminAttendance() {
        ...
        const classId = document.getElementById('admin-attendance-class')?.value;
        
        if (!date) {
            modal.alert('Validation Error', 'Please select a date', 'warning');
            return;
        }

        app.showLoading('Loading attendance...');
        
        try {
            // FIX 1: Query attendance WITHOUT foreign-key join to avoid
            // "relationship not found in schema cache" error.
            // Student names are resolved from state.students in JS instead.
            let query = supabaseClient
                .from('attendance')
                .select('id, student_id, class, date, status, marked_by, teacher_id, year_id, term_id')
                .eq('date', date);
            
            if (classId) {
                const selectedClass = state.classes.find(c => c.id === classId);
                if (selectedClass) {
                    const classString = `${selectedClass.level} - ${selectedClass.grade}`;
                    query = query.eq('class', classString);
                }
            }
            ...
            const container = document.getElementById('admin-attendance-results');
            if (container) {
                            ...
                            <table class="w-full text-left min-w-[600px]">
                                <thead class="bg-slate-50 dark:bg-slate-700 border-b border-slate-200 dark:border-slate-600">
                                    <tr>
                                        ...
                                        <th class="px-6 py-4 font-bold text-sm text-slate-700 dark:text-slate-200">Class</th>
                                        <th class="px-6 py-4 font-bold text-sm text-slate-700 dark:text-slate-200 text-center">Status</th>
                                        <th class="px-6 py-4 font-bold text-sm text-slate-700 dark:text-slate-200 text-right">Marked By</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                                    ${attendanceData.map(a => {
                                        // Resolve student name from in-memory state instead of DB join
                                        const studentRecord = state.students.find(s => s.id === a.student_id);
                                        return `
                                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                            <td class="px-6 py-4 font-semibold text-slate-800 dark:text-slate-200">${studentRecord?.name || 'Unknown Student'}</td>
                                            <td class="px-6 py-4 text-slate-800 dark:text-slate-200">${a.class}</td>
                                            <td class="px-6 py-4 text-center">
                                                <span class="px-3 py-1 rounded-full text-xs font-bold ${a.status === 'present' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}">
                                                    ${a.status === 'present' ? 'Present' : 'Absent'}
                                                </span>
                                            </td>
                                            <td class="px-6 py-4 text-right text-sm text-slate-500">${a.marked_by || 'System'}</td>
                                        </tr>`;
                                    }).join('')}
                                </tbody>
                            </table>
                        </div>
                        <div class="mt-4 flex gap-4 text-sm text-slate-600 dark:text-slate-400">
                            <span><i class="fas fa-check-circle text-emerald-500 mr-1"></i> Present: ${presentCount}</span>
                            <span><i class="fas fa-times-circle text-red-500 mr-1"></i> Absent: ${absentCount}</span>
                            <span><i class="fas fa-users text-blue-500 mr-1"></i> Total: ${attendanceData.length}</span>
                        </div>
                    `;
                }
            }
            ui.showToast('Attendance loaded', 'success');
        } catch (err) {
            modal.alert('Error', 'Failed to load attendance: ' + extractErrorMessage(err), 'error');
        } finally {
            app.hideLoading();
        }
    },
    ...
    async saveAttendance() {
        ...
        if (!teacher || !teacher.assigned_class) {
            modal.alert('Error', 'You are not assigned to a class', 'error');
            return;
        }
        ...
        try {
            ...
            myStudents.forEach(student => {
                const statusEl = document.getElementById(`attendance-status-${student.id}`);
                ...
                if (status) {
                    attendanceRecords.push({
                        ...
                        date: date,
                        status: status,
                        marked_by: state.currentUser?.full_name || 'Teacher',
            ...
            if (attendanceRecords.length === 0) {
                modal.alert('Validation Error', 'Please mark attendance for at least one student', 'warning');
                app.hideLoading();
                return;
            }
            ...
            await dataManager.loadAttendance();
        } catch (err) {
            modal.alert('Error', 'Failed to save attendance: ' + extractErrorMessage(err), 'error');
        } finally {
            app.hideLoading();
        }
    },
    ...
    async approveReportBundle(bundleId) {
        try {
            ...
            if (approvedBundle?.class) {
                ...
                const classObj = state.classes.find(c => norm(`${c.level} - ${c.grade}`) === norm(approvedBundle.class));
                const classStudents = state.students.filter(s => {
                    if (classObj && s.class_id) return s.class_id === classObj.id;
                    return norm(s.class) === norm(approvedBundle.class);
                });
    ...
    openBulkUploadWizard() {
        ...
        document.getElementById(modalId)._wizardState = { selectedClass: '', fileData: null, headers: [] };
    },
    ...
    _bulkWizardStep2(modalId) {
        ...
        const modalEl = document.getElementById(modalId);
        modalEl._wizardState.selectedClass = sel.value;
