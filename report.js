    ...
    'nursery': {
        sections: [
            ...
            },
            {
                ...
                type: 'basic_info',
                fields: [
                    { name: 'total_days',  label: 'Total School Days', type: 'number', auto: 'total'   },
                    { name: 'attendance',  label: 'Days Present',      type: 'number', auto: 'present' },
...
function renderKGFooterInfo(section, savedData, student, attendance) {
    ...
    const absentDays = Math.max(0, (parseInt(attendance.total) || 0) - (parseInt(attendance.present) || 0));
...
function renderBasicInfo(fields, savedData, autoValues) {
    ...
    // Compute days_absent from present/total so it is always consistent
    const totalDays   = parseInt(autoValues['total']   || 0);
    const presentDays = parseInt(autoValues['present'] || 0);
...
function saveCurrentReportData(isAutoSave = false) {
    ...
    try {
        ...
        // Always compute days_absent from live attendance (don't rely on readonly DOM value)
        {
            const att = state.attendanceData[student.id] || { present: 0, total: 0 };
            data.days_absent = String(Math.max(0, (parseInt(att.total) || 0) - (parseInt(att.present) || 0)));
        }
