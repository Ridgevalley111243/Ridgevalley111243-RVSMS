    ...
    'kg': {
        sections: [
            ...
            },
            {
                ...
                type: 'subjects_scored',
                headers: ['Subject','Class Score (50%)','Exam Score (50%)','Total (100%)','Remarks'],
                fields: [
            ...
            },
            {
                ...
                type: 'senses_yes_no',
                fields: [
                    { name: 'senses_colours', label: 'Colours' },
                    { name: 'senses_shapes',  label: 'Shapes'  }
                ]
            },
            {
                title: 'Physical Development',
                type: 'physical_dev',
                fields: [
                    { name: 'phys_outdoor_games',  label: 'Active & Enjoy outdoor games' },
                    { name: 'phys_muscle_control', label: 'Shows good muscle control'    },
                    { name: 'phys_neatness',       label: 'Neatness'                     },
                    { name: 'phys_leadership',     label: 'Leadership skills'            }
                ]
            },
            {
                title: 'Attendance & Promotion',
...
async function refreshTeacherAssignment() {
    ...
    try {
        ...
        // Build class label from level and grade.
        // Format: "<LEVEL> - BASIC <GRADE>"  e.g. "UPPER PRIMARY - BASIC 5"
        // The classes table has no 'name' column — name is always constructed here.
        const rawLevel = (classRecord.level || '').trim();
        const rawGrade = (classRecord.grade || '').trim();
        const classLabel = rawLevel && rawGrade
            ? `${rawLevel} - BASIC ${rawGrade}`
            : rawLevel || rawGrade || assignedClassId;
...
// Resolves formatting differences between class names stored in the DB
// (students.class) and names constructed from classes.level + classes.grade.
// Examples that must match after normalization:
//   "PRE-SCHOOL - BASIC KINDERGARTEN 2"  <->  "PRE - SCHOOL - KINDERGARTEN 2"
//   "UPPER PRIMARY - BASIC 5"            <->  "Upper Primary - Basic 5"
function normalizeClassName(className) {
    if (!className) return "";
    return className
        .toLowerCase()
        .replace(/\s*-\s*/g, "-")            // collapse spaces around hyphens: "pre - school" → "pre-school"
        .replace(/basic kindergarten/g, "kindergarten")  // "basic kindergarten" → "kindergarten"
        .replace(/basic nursery/g, "nursery")            // "basic nursery"      → "nursery"
        .replace(/\s+/g, " ")               // collapse any remaining repeated spaces
        .trim();
}
...
async function loadStudents() {
    ...
    try {
        ...
        // Canonical resolved class name: "<LEVEL> - BASIC <GRADE>"
        // Must match exactly the format built in refreshTeacherAssignment()
        const rawLevel = (classRecord.level || '').trim();
        const rawGrade = (classRecord.grade || '').trim();
        const resolvedClassName = rawLevel && rawGrade
            ? `${rawLevel} - BASIC ${rawGrade}`
            : rawLevel || rawGrade || state.assignedClassId;
...
function calculateAllPositions() {
    ...
    const subjectSection = config.sections.find(
        s => s.type === 'subjects_scored' || s.type === 'subjects_scored_full'
    );
    if (!subjectSection) return;

    const subjects = subjectSection.fields.map(f => f.name);
...
function calculateStudentTotal(studentId) {
    ...
    const config = TEMPLATE_CONFIG[state.classLevel];
    const subjectSection = config.sections.find(s => s.type === 'subjects_scored' || s.type === 'subjects_scored_full');
    if (!subjectSection) return 0;
    
    const report = state.reportsData[studentId];
    if (!report?.data) return 0;
    
    return subjectSection.fields.reduce((sum, field) => {
...
async function buildReportDocx(student, reportData) {
    ...
    // ── Build body XML ───────────────────────────────────────────────────────
    let bodyXml = buildDocxHeader(state.schoolName, logoRelId);
