...
async function initializeSupabase() {
    ...
    try {
        ...
        if (!state.assignedClass) {
            hideGlobalLoading();
            showToast('No class assigned. Please select a class from the dashboard.', 'warning');
        ...
        if (state.students.length === 0) {
            renderEmptyState('No Students Found', `No students found in class "${state.assignedClass}". Please check the class assignment.`);
            hideGlobalLoading();
            return;
        }
...
async function loadAttendanceData() {
    try {
        ...
        // Try loading from total_attendance first (summary table)
        const { data: totalData, error: totalError } = await state.supabaseClient
        ...
        if (totalError) {
            console.warn('Error loading total_attendance:', totalError);
        }
        ...
        if (dailyError) {
            console.warn('Error loading daily attendance:', dailyError);
        }
        
        // Process attendance data
        state.students.forEach(student => {
            // Try total_attendance first
            const totalRecord = totalData?.find(t => t.student_id === student.id);
            
            if (totalRecord) {
                state.attendanceData[student.id] = {
                    present: parseInt(totalRecord.present_days) || 0,
                    absent: parseInt(totalRecord.absent_days) || 0,
                    total: parseInt(totalRecord.total_days) || 0,
                    percentage: totalRecord.total_days > 0 
                        ? Math.round((totalRecord.present_days / totalRecord.total_days) * 100) 
                        : 0
                };
            } else if (dailyData) {
