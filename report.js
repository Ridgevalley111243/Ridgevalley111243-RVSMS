...
function saveCurrentReportData(isAutoSave = false) {
    ...
    try {
        ...
        // Preserve computed class_position — it's derived by calculateAllPositions()
        // and shown as a read-only badge (no [data-field] input backs it), so the
        // DOM scrape above never picks it up. Without this, it gets wiped every
        // time the form saves. Carry forward whatever was already in state.
        const existingPosition = state.reportsData[student.id]?.data?.class_position;
        if (existingPosition) data.class_position = existingPosition;
...
async function buildReportDocx(student, reportData) {
    ...
    // Templates fill these labels in as underscores that follow the label text,
    // but Word frequently splits a label or its underscore run across several
    // XML <w:t> runs (bold labels, stray spacing runs, etc.), and the exact
    // spacing/underscore-count changes whenever the template is re-formatted.
    // fillLabeledField() looks the label up in the document's plain text
    // (tags stripped) so it finds the label regardless of run-splitting, then
    // fills whatever underscore run follows it in the raw XML — so it keeps
    // working even if spacing/underscore counts change again later.
    // The header block appears twice in these templates (the visible drawing
    // + a legacy fallback copy for older Word versions), so fillLabeledField
    // fills every occurrence, not just the first.

    // Student Name — bold value
    docXml = fillLabeledField(docXml, 'Student Name:', liveValues.name, { bold: true });

    // Term / Year / Level / NO. On Roll — plain values, labels are bold in the template
    docXml = fillLabeledField(docXml, 'Term:', liveValues.term);
    docXml = fillLabeledField(docXml, 'Year:', liveValues.year);
    docXml = fillLabeledField(docXml, 'Level:', liveValues.class);
    docXml = fillLabeledField(docXml, 'NO. On Roll:', liveValues.count);

    // Date / Next Term Begins — plain values
    const dateFmt = dateVal ? formatDateForReport(dateVal) : '';
    docXml = fillLabeledField(docXml, 'Date:', dateFmt);

    const nextTermFmt = nextTermVal ? formatDateForReport(nextTermVal) : '';
    docXml = fillLabeledField(docXml, 'Next Term Begins:', nextTermFmt);

    // Class Position — bold value, added to all four templates
    docXml = fillLabeledField(docXml, 'POSITION:', reportData.class_position || '', { bold: true });
    ...
    // Same label-lookup approach as STEP 1 — resilient to the templates'
    // current spacing/bold formatting, and to future re-formatting.
    const absenceDays = Math.max(0, (parseInt(liveValues.total) || 0) - (parseInt(liveValues.present) || 0));

    // Attendance — present on Lower/Upper/KG/Creche/Nursery, plain values
    docXml = fillLabeledField(docXml, 'TOTAL SCHOOL DAYS:', liveValues.total);
    docXml = fillLabeledField(docXml, 'DAYS PRESENT:', liveValues.present);
    docXml = fillLabeledField(docXml, 'DAYS ABSENT:', String(absenceDays));

    // Interest / Conduct / Promoted To / Remarks — bold values.
    // Only the labels present in a given template's document.xml get filled;
    // fillLabeledField() is a no-op when a label isn't found, so it's safe to
    // call all of these unconditionally for every class level.
    const headRemark = (reportData.head_teacher_remarks || reportData.head_teacher_comment || '').toString().toUpperCase();

    // Lower / Upper Primary
    docXml = fillLabeledField(docXml, 'INTEREST:', reportData.interest || '', { bold: true });
    docXml = fillLabeledField(docXml, 'CONDUCT:', reportData.conduct || '', { bold: true });
    docXml = fillLabeledField(docXml, 'PROMOTED TO:', reportData.promoted_to || '', { bold: true });
    docXml = fillLabeledField(docXml, 'CLASS TEACHER’S REMARK:', reportData.class_teacher_remarks || reportData.class_teacher_comment || '', { bold: true });
    docXml = fillLabeledField(docXml, 'HEAD TEACHER’S REMARK:', headRemark, { bold: true });

    // KG
    docXml = fillLabeledField(docXml, 'SUBJECT OF INTEREST:', reportData.subject_of_interest || reportData.interest || '', { bold: true });

    // Creche / Nursery
    docXml = fillLabeledField(docXml, 'IDENTIFIED INTEREST:', reportData.interest || '', { bold: true });
    docXml = fillLabeledField(docXml, 'IDENTIFIED TALENT:', reportData.talent || '', { bold: true });
...
// ── Build a plain-text-to-XML-offset map from every <w:t> run in the document ──
// Word frequently splits a single visible phrase (e.g. "Student Name:" or
// "IDENTIFIED TALENT:") across multiple <w:t> runs — for bolding part of it,
// spell-check markers, revision tracking, etc. Searching the raw XML for the
// literal phrase misses these split cases. This builds the concatenated,
// tag-free text of the document plus a same-length array mapping each
// character back to its exact position in the raw XML, so labels can be
// found reliably regardless of how the template happens to split its runs.
function buildPlainTextMap(xml) {
    const chars = [];
    const offsets = [];
    ...
    while ((m = runRe.exec(xml)) !== null) {
        const runText  = m[1];
        const textStart = m.index + m[0].indexOf('>', m[0].indexOf('<w:t')) + 1;
        for (let i = 0; i < runText.length; i++) {
            chars.push(runText[i]);
            offsets.push(textStart + i);
        }
    }
    return { text: chars.join(''), offsets };
}

// ── Find a label anywhere in the document (even split across runs) and fill
// the underscore placeholder that follows it — spacing/underscore-count
// agnostic, so it keeps working even if a template gets re-formatted again.
// If a template repeats the same header block (some currently do, for a
// legacy-Word fallback copy), every occurrence is filled, not just the first.
// { bold: true } inserts the value as a new bold run (matching the template's
// convention for emphasized fields like Student Name, Class Position, and the
// footer remarks); otherwise the value is written into the existing
// (non-bold) placeholder run, preserving whatever formatting it already had.
//
// The underscore placeholder itself can also be split across multiple runs
// (e.g. "IDENTIFIED TALENT:" is followed by a 1-underscore run, then a
// separate 8-underscore run). We find the full underscore span in the
// plain-text view, then group it back into its raw-XML sub-ranges: the
// value is written into the first sub-range, and any further sub-ranges are
// deleted (from last to first, so earlier offsets stay valid) so no stray
// underscores are left behind.
function fillLabeledField(xml, label, value, opts = {}) {
    const bold   = !!opts.bold;
    const valStr = (value === undefined || value === null) ? '' : String(value);
    if (!valStr) return xml; // nothing to fill; leave placeholder underscores as-is

    let result = xml;
    let searchFrom = 0;

    while (true) {
        const { text, offsets } = buildPlainTextMap(result);
        const idx = text.indexOf(label, searchFrom);
        if (idx === -1) break;

        // Find the underscore span (in plain text) that follows the label,
        // allowing a little whitespace/no other visible chars in between.
        let i = idx + label.length;
        while (i < text.length && text[i] !== '_' && /\s/.test(text[i])) i++;
        const underscoreStart = i;
        while (i < text.length && text[i] === '_') i++;
        const underscoreEnd = i; // exclusive

        if (underscoreEnd === underscoreStart) {
            // No placeholder underscores follow this occurrence — move past
            // it in the plain-text search so we don't loop on it forever.
            searchFrom = idx + label.length;
            continue;
        }

        // Group the underscore span's plain-text positions into contiguous
        // raw-XML sub-ranges (a new sub-range starts wherever the XML offset
        // jumps by more than 1, i.e. a run/tag boundary sits in between).
        const ranges = [];
        let rangeStart = offsets[underscoreStart];
        let prevOffset = rangeStart;
        for (let p = underscoreStart + 1; p < underscoreEnd; p++) {
            const off = offsets[p];
            if (off !== prevOffset + 1) {
                ranges.push([rangeStart, prevOffset + 1]);
                rangeStart = off;
            }
            prevOffset = off;
        }
        ...
        // Delete every sub-range after the first, from last to first, so
        // earlier offsets (including the first sub-range) stay valid.
        for (let r = ranges.length - 1; r >= 1; r--) {
            const [s, e] = ranges[r];
            result = result.substring(0, s) + result.substring(e);
        }

        // Fill the first sub-range with the value.
        const [startIdx, endIdx] = ranges[0];
        const valEsc = xmlEsc(valStr);

        if (!bold) {
            result = result.substring(0, startIdx) + valEsc + result.substring(endIdx);
        } else {
            const beforeUnderscores = result.substring(0, startIdx);
            const afterUnderscores  = result.substring(endIdx);

            // Copy font info from the enclosing run into the new bold run
            const rStart = Math.max(beforeUnderscores.lastIndexOf('<w:r>'), beforeUnderscores.lastIndexOf('<w:r '));
            const rPrMatch = rStart !== -1
                ? beforeUnderscores.substring(rStart).match(/<w:rPr>([\s\S]*?)<\/w:rPr>/)
                : null;
            const rPrContent = rPrMatch ? rPrMatch[1] : '';
            ...
            // Close the current <w:t>/<w:r>, insert the bold run, then skip the
            // original </w:t></w:r> that follows the underscores so we don't
            // produce duplicate closing tags.
            const REST_OPEN = '</w:t></w:r>';
            const restAfter = afterUnderscores.startsWith(REST_OPEN)
                ? afterUnderscores.substring(REST_OPEN.length)
                : afterUnderscores;

            result = beforeUnderscores + REST_OPEN + boldRun + restAfter;
        }

        // Re-scan from scratch next loop (the XML shifted), continuing past
        // this occurrence in the plain text so repeated header blocks all get filled.
        searchFrom = idx + label.length;
    }
