...
async function refreshTeacherAssignment() {
    ...
    try {
        ...
        const classLabel = (
            (classRecord.level || '').trim() +
            ' - ' +
            (classRecord.grade || '').trim()
        ).trim() || assignedClassId;

        // Commit authoritative values to state
        state.assignedClassId    = assignedClassId;
        state.assignedClass      = classLabel;
        state.assignedClassLevel = (classRecord.level || '').trim();
        state.assignedClassGrade = (classRecord.grade || '').trim();

        // Sync cache
        localStorage.setItem('rv_report_class_id', assignedClassId);
        localStorage.setItem('rv_report_class',    classLabel);

        console.log('[refreshTeacherAssignment] Successfully resolved assignment:', {
            assignedClassId: state.assignedClassId,
            assignedClass:   state.assignedClass,
            level: state.assignedClassLevel,
            grade: state.assignedClassGrade
        });
...
async function loadStudents() {
    ...
    try {
        ...
        // Canonical resolved class name: UPPER(level) + ' - ' + UPPER(grade)
        const resolvedClassName = (
            (classRecord.level || '').trim().toUpperCase() +
            ' - ' +
            (classRecord.grade || '').trim().toUpperCase()
        ).trim();

        console.log('[loadStudents] Resolved class name:', resolvedClassName);

        // ── STEP 3: Fetch students and apply strict exact-match filter ─────────
        ...
        const filtered = (allStudents || []).filter(s => {
            const studentClass = (s.class || '').trim().toUpperCase();
            const matches = studentClass === resolvedClassName;

            if (matches) {
                console.log(`[loadStudents] ACCEPTED [Exact class match]: ${s.name} | class="${s.class}"`);
            } else {
                console.log(`[loadStudents] REJECTED [Wrong class]: ${s.name} | class="${s.class}" | expected="${resolvedClassName}"`);
            }
...
async function loadSavedReports() {
    ...
    try {
        ...
        console.log('[loadSavedReports] After seeding, total state entries:',
