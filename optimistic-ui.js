...
// ── 9a. addStudent ────────────────────────────────────────────────────────────────
// FIX: The original patch read the raw UUID from the class <select> and saved it
// directly into students.class, violating the no_uuid_class_check constraint and
// bypassing the buildClassName() / assertNotUUID() guards in app.js.
//
// This patch now:
//   1. Reads classId (UUID) from the select — used only as a lookup key.
//   2. Resolves the class object from state.classes (same as app.js addStudent).
//   3. Calls buildClassName() to produce the canonical readable name (e.g.
//      "LOWER PRIMARY - BASIC 1") — this is what gets written to students.class.
//   4. Includes class_id in the insert so the FK column is always populated.
//   5. Uses the readable name for the optimistic temp record too, so the
//      in-flight UI shows the correct label rather than a raw UUID.
...
if (_origAddStudent) {
    actions.addStudent = async function() {
        ...
        // The select emits the class UUID as its value — resolve it below
        const classId         = document.getElementById('student-class')?.value;
        const parentPhone     = document.getElementById('student-parent-phone')?.value?.trim();

        if (!admissionNumber || !name || !dob || !classId) {
            return modal.alert('Validation Error', 'Please fill in Admission Number, Name, Date of Birth, and Class', 'warning');
        }

        // FIX: resolve UUID → canonical class name, never write UUID into students.class
        const classObj = state.classes.find(c => c.id === classId);
        if (!classObj) {
            return modal.alert('Validation Error', 'Selected class not found. Please refresh and try again.', 'warning');
        }
        const resolvedClassName = buildClassName(classObj);
        assertNotUUID(resolvedClassName, 'optimistic-addStudent');
        ...
        const tempStudent = {
            ...
            age: age || null,
            class: resolvedClassName,   // FIX: readable name, not UUID
            class_id: classId,          // FIX: FK column always included
        ...
        await withOptimistic(async () => {
            try {
                const { data, error } = await supabaseClient.from('students').insert([{
                    ...
                    dob,
                    age: age || null,
                    class: resolvedClassName,   // FIX: canonical name satisfies constraint
                    class_id: classId,          // FIX: FK column included
                    parent_phone: parentPhone || null
                }]).select().single();
...
// ── 9c. addClass ──────────────────────────────────────────────────────────────────
// IMPORTANT: app.js addClass() already contains the full restore-vs-create logic:
// it normalises the level+grade input, checks Supabase for a soft-deleted match,
// and either restores the original row (preserving its UUID and all FK relationships)
// or inserts a fresh row.  This patch must NOT duplicate that logic or call
// supabaseClient directly — doing so bypasses restoration and creates a new UUID,
// which breaks student/teacher/attendance/report reconnection.
//
// The optimistic layer's role here is only:
//   1. Validate inputs and show instant "pending" feedback.
//   2. Suppress the full-page loading overlay so the UI stays responsive.
//   3. Delegate the actual backend work to _origAddClass() with the DOM intact.
//   4. After the backend completes, clean up the temp placeholder and run a
//      silent background refresh so all class-dependent views stay in sync.
//
// ORDERING CONSTRAINT: ui.route() must NOT be called before _origAddClass().
// The patched ui.route() immediately replaces the entire view-content DOM with a
// skeleton and re-renders renderClasses() on the next tick.  This destroys the
// #class-level and #class-grade input elements.  When _origAddClass() then runs
// and reads those inputs it gets empty strings, fails its own validation, and
// fires the spurious "Please fill all fields" alert — the exact bug this fixes.
// Solution: read and validate values here first, push the optimistic placeholder
// to state (so the list updates visually), then await _origAddClass() while the
// DOM is still intact.  Only navigate away after _origAddClass() resolves.
const _origAddClass = actions.addClass?.bind(actions);
if (_origAddClass) {
    actions.addClass = async function() {
        // Read values from the DOM immediately — before any state mutation or
        // route change that could destroy the form elements.
        const level = document.getElementById('class-level')?.value?.trim();
        const grade = document.getElementById('class-grade')?.value?.trim();

        // Validate here so the optimistic layer gives fast feedback; _origAddClass
        // will also validate, but it must see the same non-empty values.
        if (!level || !grade) return modal.alert('Validation Error', 'Please fill all fields', 'warning');

        // Push a temporary placeholder into state so the classes list already
        // shows the new entry while the network call is in flight.  Do NOT call
        // ui.route() yet — the form must remain in the DOM for _origAddClass().
        const tempId = 'temp_' + Date.now();
        const tempClass = { id: tempId, level, grade, _optimistic: true, created_at: new Date().toISOString() };
        state.classes.push(tempClass);
        ui.showToast('Saving class...', 'info');

        await withOptimistic(async () => {
            try {
                // Delegate entirely to the app.js implementation.  The DOM inputs
                // are still present at this point, so _origAddClass() can read them,
                // run its own normalised restore-vs-create logic, reload classes and
                // students in the correct order, and call ui.route('classes') itself.
                await _origAddClass();

                // _origAddClass has now reloaded classes and students and navigated
                // to the classes view.  Remove the temp placeholder — the real row
                // (restored with its original UUID, or freshly inserted) is already
                // in state.classes from _origAddClass's internal loadClasses() call.
                state.classes = state.classes.filter(c => c.id !== tempId);

                // Run a silent background refresh of all class-dependent collections
                // so teacher dashboards, attendance pages, and report pages reflect
                // the restored UUID relationships without requiring a manual reload.
                silentRefresh([
                ...
                // _origAddClass calls ui.route('classes') and ui.showToast() itself,
                // so no additional route/toast call is needed here.
            } catch (err) {
                // Roll back the temp placeholder on any failure and navigate so the
                // user sees the current classes list rather than a stale skeleton.
                state.classes = state.classes.filter(c => c.id !== tempId);
                ui.route('classes');
                modal.alert('Error', extractErrorMessage(err), 'error');
            }
...
// ── 9d. deleteClass ───────────────────────────────────────────────────────────────
// Soft-delete only: marks the class as is_deleted=true instead of physically
// removing the row. This preserves all foreign key relationships (students,
// attendance, reports, teacher assignments) and prevents constraint errors.
...
if (_origDeleteClass) {
    actions.deleteClass = function(id) {
        modal.confirmDelete('this class', async () => {
            ...
            // Optimistically remove the class from the active frontend view
            ...
            await withOptimistic(async () => {
                try {
                    ...
                    // Never use .delete() here — it triggers foreign key constraint errors
                    // because students, attendance records, and reports still reference this class.
                    const { error } = await supabaseClient
                        .from('classes')
                        .update({ is_deleted: true })
                        .eq('id', id);
                    if (error) throw error;
                    silentRefresh([() => dataManager.loadClasses()]);
                } catch (err) {
