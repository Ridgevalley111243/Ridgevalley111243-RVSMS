...
const PROMOTION_MAP = {
    ...
    'Form 4':   'Form 5',
    'Form 5':   'Form 6',
    // "Basic X" style  ← ADDED: was missing, causing all Basic-named classes to fail
    'Basic 1':  'Basic 2',
    'Basic 2':  'Basic 3',
    'Basic 3':  'Basic 4',
    'Basic 4':  'Basic 5',
...
const GRADUATION_GRADES_LC = new Set([
    ...
    // Kindergarten terminal (KG 2 / Kindergarten 2)
    'kg 2', 'kindergarten 2',
...
function getNextClass(student) {
    ...
    // FIX: case/space-insensitive promotion map lookup.
    // The old code did an exact-key lookup on PROMOTION_MAP, which silently
    // returned undefined for grades like "Basic 1" (not in the old map) or
    // "basic 1" / "Basic  1" (casing/spacing mismatch).  We now normalise
    // the grade before lookup AND scan the map case-insensitively as a fallback.
    const rawGrade   = (currentClass.grade || '').trim();
    const rawGradeLc = rawGrade.toLowerCase().replace(/\s{2,}/g, ' ');

    let nextGrade = PROMOTION_MAP[rawGrade]; // fast path: exact match
    if (!nextGrade) {
        // Slow path: case + whitespace insensitive scan
        const entry = Object.entries(PROMOTION_MAP).find(
            ([k]) => k.toLowerCase().replace(/\s{2,}/g, ' ') === rawGradeLc
        );
    ...
    const currentLevelLc = (currentClass.level || '').trim().toLowerCase();

    // Only search active (non-soft-deleted) classes — same filter as _resolveClassObj
    const activeClasses = (state.classes || []).filter(c => !c.deleted_at && c.status !== 'deleted');

    // Primary match: same level AND matching grade (case/space insensitive)
    const sameLevel = activeClasses.find(c =>
        (c.level || '').trim().toLowerCase() === currentLevelLc &&
        (c.grade || '').trim().toLowerCase().replace(/\s{2,}/g, ' ') === nextGradeLc
    );
    if (sameLevel) return sameLevel;

    // FIX: level-relaxed fallback.
    // Some schools use different level labels across class years (e.g.
    // "Lower Primary" for Basic 1-3 and "Upper Primary" for Basic 4-6).
    // The strict level === level check caused getNextClass to return null
    // even when the next-grade class existed under a different level name.
    // This fallback finds it by grade alone.
    const anyLevel = activeClasses.find(c =>
        (c.grade || '').trim().toLowerCase().replace(/\s{2,}/g, ' ') === nextGradeLc
    );
    ...
    async promoteStudents(students, fromAY, toAY) {
        ...
        for (const student of students) {
            try {
                const nextClass = getNextClass(student);
                if (!nextClass) {
                    results.skipped.push({ student, reason: 'No next class mapping found' });
                    continue;
                }
...
const promotionViews = {
    ...
    async renderPromotion({ silent = false } = {}) {
        ...
        promoCandidates.forEach(s => {
            ...
            const label = classObj ? `${classObj.level} - ${classObj.grade}` : (s.class || 'Unassigned');
                ...
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:12px;">
                    <div style="display:flex;align-items:center;gap:12px;">
                        ...
                        <div>
                            ...
                            <p style="font-size:12px;color:var(--rv-muted);margin:2px 0 0;">${graduates.length} student(s) in final year — will be archived permanently</p>
                        </div>
                    </div>
                            ...
                            <div style="border:1px solid var(--rv-border);border-radius:12px;overflow:hidden;">
                                ...
                                <div id="class-group-${groupId}" style="display:none;">
                                    ${students.map(s => {
                                        ...
                                        const nextLbl = nextC ? `${nextC.level} - ${nextC.grade}` : '—';
                                        return `
